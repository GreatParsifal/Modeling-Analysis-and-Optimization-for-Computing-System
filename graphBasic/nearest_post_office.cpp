/*
描述
城市P被划分成了N*M个大小相同的街区，其中有些街区有邮局。我们可以用坐标(i, j) (1 <= i <= N, 1 <= j <= M)来表示一个街区。
坐标为(i, j)的街区能够去到坐标为(i-1, j), (i+1, j), (i, j-1), (i, j+1)这4个相邻的街区 (处于边界的街区不能够越界)，且距离都是1。现在你需要求出每个街区到最近的邮局的距离。
输入
